...
def gpt_simulate(action_text):
    ...
    system_prompt = (
        ...
        "Consider the nature of the bill and political dynamics when determining support changes: "
        "- Controversial/divisive bills (gun control, abortion, major ideological issues) face natural resistance from moderates, swing-district members, and the opposing party. Support builds more slowly and may stall or decline when opposition mobilizes. "
        "- In the Senate, reaching 60 votes often requires bipartisan support or near-unanimous party unity, making coalition-building inherently challenging for partisan legislation. Be realistic about the difficulty of getting 60 senators on board for controversial bills. "
        "- Members from EVEN districts are cautious about controversial votes that could hurt reelection. They may withhold support or demand compromises. "
        "- Bipartisan bills (infrastructure, disaster relief, national security) build support more easily across the aisle and should see faster coalition growth. "
        "- Adjust the pace and magnitude of support changes based on these political realities, not just the member's individual actions. "
        ...
        "**CRITICAL: When support is well above threshold (65%+ in House, 70%+ in Senate), progress MUST be 25-30% per turn when the member takes decisive action (lobbying leadership, demanding votes, pressuring committees, scheduling floor votes). Do not give less than 25% progress in these high-support scenarios with strong actions.** "
        "- When support is strong and the member takes decisive action, reward with substantial progress. Do not artificially slow down bills that have overwhelming support. "
        "**SUPPORT CHANGE GUIDELINES:** "
        "- In the first 3 turns, support changes should be modest (±3-8) as the member is still building initial coalitions and gauging interest. Larger support swings (±10-15) come in mid-to-late game once the bill has momentum and the member has established credibility. "
        "- Support should DECREASE (negative values -3 to -8 early, -5 to -15 later) when: the bill contradicts the member's party ideology, "
        ...
        "- Support should INCREASE (positive values +3 to +8 early, +5 to +15 later) when: the bill strongly aligns with party priorities, "
        ...
        "**DISTRICT ALIGNMENT AND CONTROVERSY:** "
        "Always consider the member's district lean AND the bill's controversy level when evaluating public approval and reelection risk: "
        ...
        "- In EVEN districts: Approval changes depend heavily on controversy level. For divisive issues, gains from your base are offset by losses from the other side, resulting in small net changes (±2-5) or even negative changes if swing voters are alienated. Controversial positions in EVEN districts significantly INCREASE reelection risk (+3-8 per turn). "
        "- For bipartisan/consensus bills in any district: Public approval typically increases as constituents appreciate pragmatic governance. "
        ...
        "The district's partisan lean AND the bill's divisiveness should be PRIMARY factors in public approval and reelection risk changes. "
